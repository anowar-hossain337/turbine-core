...
#ifndef TURBINECORE_NEARESTNEIGHBOURFIELDDISTRIBUTOR_H
#define TURBINECORE_NEARESTNEIGHBOURFIELDDISTRIBUTOR_H
...
namespace turbine_core {
    ...
    namespace projectors {
        ...
        class NearestNeighbourFieldDistributor {

        public:

            HOST_DEVICE_PREFIX NearestNeighbourFieldDistributor( const blockforest::BlockInfo & blockInfo,
                                                                 Field_T * baseField )
                    : blockInfo_(&blockInfo), baseField_(baseField)
            {}

            template<typename ForwardIterator_T>
            HOST_DEVICE_PREFIX void distribute( const Vector3<real_t> & position, ForwardIterator_T distributeValueBegin ) {
                distribute( position[0], position[1], position[2], distributeValueBegin );
            }

            template< typename ForwardIterator_T >
            HOST_DEVICE_PREFIX void distribute( const real_t & x, const real_t & y, const real_t & z, ForwardIterator_T distributeValueBegin ) {
...
//                }
                ...
                if (!isBoundaryCell) {
                    for( uint_t f = 0; f < baseField_->fSize(); ++f ) {
                        baseField_->get( containingCell, f) = *distributeValueBegin;
...
//                                    }
                                    ...
                                    if (isDomainCell) {
                                        for (uint_t f = 0; f < baseField_->fSize(); ++f) {
                                            baseField_->get(curCell, f) = *distributeValueBegin;
                                            ++distributeValueBegin;
                                        }
                                        return;
                                    }
...
#endif //TURBINECORE_NEARESTNEIGHBOURFIELDDISTRIBUTOR_H
