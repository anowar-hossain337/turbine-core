...
//        projectors::GaussianFunction5<>::setSigma(real_t(7.07106781));
        ...
        auto communication = std::make_shared<walberla::blockforest::communication::UniformBufferedScheme<CommunicationStencil_T>>(blocks);
        auto pdfPackInfo = std::make_shared< walberla::lbm_generated::UniformGeneratedPdfPackInfo<PdfField_T> >(pdfFieldCpuID);
        communication->addPackInfo( pdfPackInfo );
        auto densityPackInfo = std::make_shared< walberla::field::communication::PackInfo<ScalarField_T> >(densityFieldCpuID);
        communication->addPackInfo( densityPackInfo );
        auto velocityPackInfo = std::make_shared< walberla::field::communication::PackInfo<VectorField_T> >(velocityFieldCpuID);
